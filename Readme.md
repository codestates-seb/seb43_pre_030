...
|이름|맡은 부분|
|---|---|
|김진완|헤더, 메인 페이지 인터페이스 및 기능, 검색 기능, 전체 기능 보수 |
|강수암|페이지 중 aside바 구현, 로그인페이지, 질문상세페이지 인터페이스 및 기능구현|
|이제윤| |
|유호균|피그마, 페이지 중의 footer, /register (회원가입 페이지), /users (회원정보 페이지)|
...
|이름|맡은 부분|
|---|---|
|한동민|USER CRUD 구현, 로그인 인증 개발(JWT 발급,인증), OAuth2.0으로 로그인 인증 처리(구글, +@), Spring Security  |
|윤승현| |
|강지산|답변 CRUD 구현, 댓글 CRUD 구현, Rest Docs 문서화 기능 구현, AWS 배포 환경 구축|
